            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body" style="min-height: 400px;">
                    <div id="main4" class="layui-col-sm12" style="height: 400px;">
                        <table class="layui-table layui-form" id="knowledge" lay-filter="knowledge"></table>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    layui.use(['table', 'layer','upload'], function () {
        ...
        var tableIns = table.render({
            elem: '#knowledge'
            , height: 400
            , url: '${pageContext.request.contextPath}/knowledge/selKnowledge' //数据接口
            , page: true //开启分页
            , cols: [[ //表头
        ...
        table.on('tool(knowledge)', function (obj) {
            ...
            if (layEvent === 'update') {    //修改
                $.ajax({
                    url: "${pageContext.request.contextPath}/knowledge/findKnowledgeById",
                    type: "Post",
                    data: {"id":id},
                    dataType: "text",
                    success:function (data) {
                    var knowledge=JSON.parse(data);
                    $("#update_title").val(knowledge.title);
                    $("#update_type").val(knowledge.type);
                    }
                });
                layer.open({
                    title:"知识修改",
                    ...
                    btn1:function (index) {
                      ...
                      var type=$("#update_type").val();
                        $.ajax({
                            url: "${pageContext.request.contextPath}/knowledge/updateKnowledge",
                            type: "Post",
                            data: {"id":id,"title":title,"type":type},
                            dataType: "text",
                            beforeSend:function(){
                                return confirm("确认修改"+title+"？")
                            },
                            success:function (data) {
                                if (data=="1"){
                                    table.reload('knowledge',{
                                        url: '${pageContext.request.contextPath}/knowledge/selKnowledge'
                                        ,height: 400
                                        ,page:{
                                            curr:1
                                        }
                ...
                $.ajax({
                    url: "${pageContext.request.contextPath}/knowledge/deleteKnowledge",
                    type: "Post",
                    data: {"id":id},
                    dataType: "text",
                    beforeSend:function(){
            ...
            }else if (layEvent === 'document'||layEvent==='image') {
                window.open(url);
        ...
        $("#addKnowledge").on('click',function () {
            ...
            upload.render({
                elem: '#test8'
                ,url: '${pageContext.request.contextPath}/knowledge/addKnowledge' //改成您自己的上传接口
                ,auto: false
                ,accept:"file"
                ,multiple: true
                ,before:function (obj) {
...
<script type="text/html" id="barDemo">
    {{#  if(d.type=="文章"){ }}
    <button class="layui-btn layui-btn-normal" type="button" lay-event="document">阅读文章</button>
    {{#  } }}
    {{#  if(d.type=="图片"){ }}
    <button class="layui-btn layui-btn-normal" type="button" lay-event="image">查看图片</button>
    {{#  } }}
    {{#  if(d.type=="视频"){ }}
    <button class="layui-btn layui-btn-normal" type="button" lay-event="video">播放视频</button>
    {{#  } }}
...
<div id="play_div" style="width:700px;height:260px;text-align: center;display: none">
    <div id="video">
        <video src="https://blz-videos.nosdn.127.net/1/OverWatch/AnimatedShots/Overwatch_AnimatedShot_CinematicTrailer.mp4" id="videoShow"></video>
    </div>
</div>
</html>
